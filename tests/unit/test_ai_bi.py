...
def test_ai_bi_list_files():
    ...
    assert "resources/catalogs/catalog.yml" not in files
...
def test_ai_bi_scaffold(tmp_path: Path):
    ...
    assert not (project_dir / "resources" / "catalogs" / "catalog.yml").exists()
...
def test_ai_bi_scaffold_uses_existing_catalog(tmp_path: Path):
    from dpa.accelerators import get_accelerator
    ...
    assert "default: workspace" in bundle
    assert "catalogs:" not in bundle
...
def test_ai_bi_scaffold_renders_setup_notebook(tmp_path: Path):
    ...
    assert 'dbutils.widgets.text("catalog", "workspace")' in nb
...
def test_ai_bi_scaffold_renders_dashboard_catalog(tmp_path: Path):
    ...
    assert all("workspace.tpch_metrics" in q for q in queries)
