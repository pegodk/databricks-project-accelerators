...
def test_medallion_dbt_list_files():
    ...
    assert "resources/catalogs/catalogs.yml" not in files
...
def test_medallion_dbt_scaffold(tmp_path: Path):
    ...
    assert not (project_dir / "resources" / "catalogs" / "catalogs.yml").exists()
...
def test_medallion_dbt_scaffold_does_not_create_catalogs(tmp_path: Path):
    from dpa.accelerators import get_accelerator
    ...
    assert not (project_dir / "resources" / "catalogs").exists()
